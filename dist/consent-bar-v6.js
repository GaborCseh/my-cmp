 ...
 * v3.0: Reject button toggle on first view; banner sizing customization.
 * v4.0: Consent default enable/disable toggle (controlled via GTM template).
 * v5.0: Consent default moved to separate Custom HTML tag (visible in console,
 *        no duplicate firing). Template now handles only banner + updateConsentState.
 * v6.0: Boot split into two phases —
 *        1) bootConsent()  runs synchronously on script parse (no DOM needed):
 *           reads the cookie and fires the dataLayer event immediately.
 *        2) initUI()       runs on DOMContentLoaded: styles, banner, revisit button.
 *        On returning visits the consent update itself is issued by the GTM template
 *        (updateConsentState, zero latency). The script skips its own duplicate
 *        gtag('consent','update') when cfg.tur === 1. Standalone usage (no GTM
 *        template) keeps the old behaviour and still issues the update itself.
 * ES5 compatible.
 */
(function (w, d) {
  'use strict';
  ...
  // Text defaults (English). Any field can be overridden via cfg.tx.
  var T = {
    title:        tx.title        !== undefined ? tx.title        : 'Our Site Uses Cookies',
    desc:         tx.desc         !== undefined ? tx.desc         : 'We use essential cookies to ensure the proper functioning of our website and, with your consent, optional cookies to analyze traffic and support marketing activities. You can accept all cookies, reject non-essential ones, or customize your preferences at any time.',
    btnAccept:    tx.btnAccept    !== undefined ? tx.btnAccept    : 'Accept All',
    btnReject:    tx.btnReject    !== undefined ? tx.btnReject    : 'Decline',
    btnManage:    tx.btnManage    !== undefined ? tx.btnManage    : 'Customize',
    btnSave:      tx.btnSave      !== undefined ? tx.btnSave      : 'Save',
    catNecLabel:  tx.catNecLabel  !== undefined ? tx.catNecLabel  : 'Necessary',
    catNecDesc:   tx.catNecDesc   !== undefined ? tx.catNecDesc   : 'Necessary cookies are required for the website to function and cannot be switched off. They enable core functionality such as security, network management, and accessibility.',
    catStatLabel: tx.catStatLabel !== undefined ? tx.catStatLabel : 'Statistics',
    catStatDesc:  tx.catStatDesc  !== undefined ? tx.catStatDesc  : 'Statistics cookies help us understand how visitors interact with our website by collecting and reporting information about usage. This data is used to improve how the website works.',
    catMktLabel:  tx.catMktLabel  !== undefined ? tx.catMktLabel  : 'Marketing',
    catMktDesc:   tx.catMktDesc   !== undefined ? tx.catMktDesc   : 'Marketing cookies are used to track visitors across websites and display relevant advertisements. They measure the effectiveness of advertising campaigns.',
    privacyLink:  tx.privacyLink  !== undefined ? tx.privacyLink  : 'Privacy Policy',
  ...
  var privacyPolicyUrl  = cfg.ppDf || '/privacy-policy';
  ...
  // Template-side consent restore (v6.0)
  // 1 = the GTM template already called updateConsentState() from the cookie,
  //     so the script must NOT issue a second gtag('consent','update').
  // 0 / undefined = standalone usage — the script issues the update itself.
  var templateDidRestore = cfg.tur === 1;

  // Layout (v3.0)
  var showRejectBtn      = cfg.srj !== 0;
  var bannerMaxWidth     = cfg.mw  || '560px';
  var bannerBorderRadius = cfg.br  || '12px';
  var bannerPadding      = cfg.pd  || '24px';
  var cornerOffset       = cfg.co  || '16px';
  ...
  function createRevisitBtn() {
    ...
    var posStyle = revisitBtnPosition === 'bottomright'
      ? 'right:' + cornerOffset + ';bottom:' + cornerOffset + ';'
      : 'left:' + cornerOffset + ';bottom:' + cornerOffset + ';';
  ...
  function injectStyles() {
    ...
    if (bannerPosition === 'center') {
      ...
      pos = '#' + BANNER_ID + '{bottom:0;left:0;width:100%;max-width:100%;' +
            'border-radius:' + bannerBorderRadius + ' ' + bannerBorderRadius + ' 0 0}';
    } else if (bannerPosition === 'bottomleft') {
      pos = '#' + BANNER_ID + '{bottom:' + cornerOffset + ';left:' + cornerOffset + '}';
    }
    ...
    var css = [
      ...
      '#' + BANNER_ID + '{display:none;position:fixed;z-index:999999;box-sizing:border-box;' +
        ...
        'max-width:' + bannerMaxWidth + ';width:calc(100% - 32px);' +
        'border-radius:' + bannerBorderRadius + ';padding:' + bannerPadding + ';' +
  ...
  function buildHTML() {
    ...
    // Initial view — reject button is optional (v3.0)
    var vi = '<div id="dc-vi">' +
      '<div class="dct">'  + T.title + '</div>' +
      '<div class="dctx">' + T.desc  + '</div>' +
      '<div class="dcbt">' +
        '<button class="' + pb + '" id="dc-acc">' + T.btnAccept + '</button>' +
        (showRejectBtn ? '<button class="' + sb + '" id="dc-rej">' + T.btnReject + '</button>' : '') +
        '<button class="' + sb + '" id="dc-mng">' + T.btnManage + '</button>' +
      '</div>' +
      footer() +
    '</div>';

    // Customize view — always shows both accept and reject
  ...
  function readXDConsent() {
    ...
    return { ac: sm && sm[1] === '1', mc: mm && mm[1] === '1' };
  }

  // ── Init — phase 1 (v6.0) ──────────────────────────────────────────────────
  // Runs synchronously as soon as the script is parsed. Needs no DOM, so the
  // consent signal is not held back by DOMContentLoaded.
  var bootMode = 'banner';   // 'xd' | 'restored' | 'banner'
  ...
  function bootConsent() {
    ...
    if (xd) {
      state.ac = xd.ac ? 1 : 0;
      state.mc = xd.mc ? 1 : 0;
      writeConsent(xd.ac, xd.mc);
      // Cross-domain consent is new information the GTM template cannot know
      // about, so the script always issues this update itself.
      gtagUpdate(xd.ac, xd.mc);
      fireDLEvents(xd.ac, xd.mc, true);
      bootMode = 'xd';
      return;
    }
    ...
    if (existing !== null) {
      ...
      state.ac = ac ? 1 : 0;
      state.mc = mc ? 1 : 0;
      // v6.0: the GTM template already restored this exact state via
      // updateConsentState() at Consent Init — do not send it twice.
      if (!templateDidRestore) { gtagUpdate(ac, mc); }
  ...
  // ── Init — phase 2 (v6.0) ──────────────────────────────────────────────────
  // Needs the DOM: styles, banner markup, revisit button.
  function initUI() {
    injectStyles();

    if (bootMode === 'xd') {
      if (enableXD && crossDomainHosts) { patchXDLinks(state.ac === 1, state.mc === 1); }
      showRevisitBtn();
      return;
    }

    if (bootMode === 'restored') {
      showRevisitBtn();
      return;
    }
  ...
  w[globalObjectName] = {
    ...
    clearConsent: function () {
      ...
      state.ac = 0; state.mc = 0; state.rv = 0;
      hideRevisitBtn();
      var bn = d.getElementById(BANNER_ID);
      var ov = d.getElementById(OVERLAY_ID);
    ...
    hasConsent:  function () { return readConsent() !== null; },
    setLanguage: function () { /* texts are set via config.tx in GTM template */ },
    getLanguage: function () { return 'custom'; }
  };

  // ── Boot (v6.0) ────────────────────────────────────────────────────────────
  // Phase 1 fires the consent signal right away — no waiting for the DOM.
  bootConsent();

  // Phase 2 builds the UI once the DOM is available.
  if (d.readyState === 'loading') {
    d.addEventListener('DOMContentLoaded', initUI);
  } else {
    initUI();
  }
